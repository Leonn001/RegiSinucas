...
class DashboardService {
    ...
    async getLucroPorDistrito({ cidade, ano, mes }) {
        if (!cidade) {
            return [];
        }
        ...
        if (ano) {
            whereClause[Op.and] = [where(fn('EXTRACT', fn('YEAR FROM', col('LeituraMesa.data_leitura'))), ano)];
        }
        if (mes) {
            whereClause[Op.and] = [
                ...(whereClause[Op.and] || []),
